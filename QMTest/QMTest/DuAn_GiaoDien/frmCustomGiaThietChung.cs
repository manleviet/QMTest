...
namespace DuAn_GiaoDien
{
    public partial class frmCustomGiaThietChung : Form
    {
        ...
        bool Show;
        public frmCustomGiaThietChung(bool show)
        {
            InitializeComponent();
            Show = show;
        }
        ...
        private void frmCustomGiaThietChung_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                this.Size = new Size(418, 452);
                gridControl1.DataSource = gtc_bll.GiaThietChung_Select();
                splitContainerControl3.SplitterPosition = 365;
            }
        }
                ...
                if (XtraMessageBox.Show("Bạn chắc chắn xóa ?", "Thông báo", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
                {
                    ...
                    if (kq > 0)
                    {
                        ...
                        try
                        {
                            ...
                            sys.MoTa = string.Format("{0} {1}", "Xóa giả thiết chung có mã là",gtc.IdGiaThietChung);
            ...
            if (XtraMessageBox.Show("Bạn chắc chắn cập nhật ?", "Thông báo", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
            {
                ...
                if (kq > 0)
                {
                    ...
                    try
                    {
                        ...
                        sys.Gv.IdGiaoVien = frm_DangNhap.GetIdGiaoVien;
                        sys.Gv.TenGiaoVien = frm_DangNhap.GiaoVien.TenGiaoVien;
                        sys.MoTa = string.Format("{0} {1}", "Cập nhật giả thiết chung có mã là", gtc.IdGiaThietChung);
                        sys_bll.SystemLog_Insert(sys);
                    }
                    catch { }
                }
        ...
        private void btnChon_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    SetIdGiaThietChung = int.Parse(gridView1.GetFocusedRowCellValue("IdGiaThietChung").ToString());
                    MessageBox.Show(GetIdGiaThietChung.ToString());
                }
                catch { }
                this.Close();
            }
