...
namespace DuAn_GiaoDien
{
    public partial class UcDanhMucKhoa : UserControl
    {
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            {
                if (XtraMessageBox.Show(String.Format("Bạn muốn xóa khoa {0}?", k.TenKhoa), "Thông báo", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    int kq = k_bll.Khoa_Delete(k);
                    if (kq > 0)
                    {
                        XtraMessageBox.Show(string.Format("Xóa khoa {0} thành công!", k.TenKhoa), "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        UcDanhMucKhoa_Load(sender, e);
                    }
                    else XtraMessageBox.Show(string.Format("Xóa khoa {0} thất bại!", k.TenKhoa), "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
